read_targets <- list(
  ...
  tar_target(average_duplicates, function(df, type) {
    ...
    if (type == "trib") {
      id_cols <- c(id_cols, "huc")
    }

    data_cols <- c()
    if (type == "trib") {
      data_cols <- c(data_cols, "discharge")
    }
    data_cols <- c(data_cols, "chl")
    if (type != "trib") {
      data_cols <- c(data_cols, "chl_field")
    } # no tribs have chl sensor data
    data_cols <- c(
      data_cols,
      "tss",
      "turb",
