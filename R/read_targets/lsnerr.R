lsnerr_targets <- list(
  ...
  tar_target(
    qc_lsnerr_wq,
    Vectorize(function(value, flag) {
      if (is.na(flag)) {
        return(value)
      } else if (str_detect(flag, "1") | str_detect(flag, "-3")) {
        # remove values that are suspect (1) or did not pass QC (-3)
        return(NA)
      } else {
        return(value)
      }
  ...
  tar_target(
    ...
    Vectorize(function(value, flag) {
      if (is.na(flag)) {
        return(value)
      } else if (str_detect(flag, "-4")) {
        # set NDs to be .5 the MDL, reported values are the MDL for that year
        return(value * 0.5)
      } else if (str_detect(flag, "G") | str_detect(flag, "-3")) {
        # remove values that are suspect (1) or rejected (-3) due to QC issues, but leave in samples that were suspect due to delayed analysis
        return(NA)
      } else {
        return(value)
      }
  ...
  tar_target(get_lkswq_file, function() {
    ...
    for (file in list.files(dir)) {
      lkswq <- bind_rows(
        lkswq,
        read_csv(str_c(dir, file, sep = "/"), show_col_types = FALSE)
      )
    }
    return(lkswq[1:30])
  }),
  ...
  #' Science Collaborative file includes 2024 LSNERR SWMP nutrient data
  #'
  #' @return A file path.
  tar_target(
    nerr_stations_file,
    "ref/nerr_sampling_stations.csv",
    format = "file"
  ),
  tar_target(
    scico_stations_file,
    "ref/SciCollabLocations.csv",
    format = "file"
  ),

  tar_target(
    lksnut12_file,
    "raw_data/lsnerr/lksnut/lksnut2012.csv",
    format = "file"
  ),
  tar_target(
    lksnut13_file,
    "raw_data/lsnerr/lksnut/lksnut2013.csv",
    format = "file"
  ),
  tar_target(
    lksnut14_file,
    "raw_data/lsnerr/lksnut/lksnut2014.csv",
    format = "file"
  ),
  tar_target(
    lksnut15_file,
    "raw_data/lsnerr/lksnut/lksnut2015.csv",
    format = "file"
  ),
  tar_target(
    lksnut16_file,
    "raw_data/lsnerr/lksnut/lksnut2016.csv",
    format = "file"
  ),
  tar_target(
    lksnut17_file,
    "raw_data/lsnerr/lksnut/lksnut2017.csv",
    format = "file"
  ),
  tar_target(
    lksnut18_file,
    "raw_data/lsnerr/lksnut/lksnut2018.csv",
    format = "file"
  ),
  tar_target(
    lksnut19_file,
    "raw_data/lsnerr/lksnut/lksnut2019.csv",
    format = "file"
  ),
  tar_target(
    lksnut20_file,
    "raw_data/lsnerr/lksnut/lksnut2020.csv",
    format = "file"
  ),
  tar_target(
    lksnut21_file,
    "raw_data/lsnerr/lksnut/lksnut2021.csv",
    format = "file"
  ),
  tar_target(
    lksnut22_file,
    "raw_data/lsnerr/lksnut/lksnut2022.csv",
    format = "file"
  ),
  tar_target(
    lksnut23_file,
    "raw_data/lsnerr/lksnut/lksnut2023.csv",
    format = "file"
  ),

  tar_target(
    scico_file,
    "raw_data/lsnerr/scicollabdata021125.csv",
    format = "file"
  ),
  ...
  tar_target(
    ...
    read_csv(
      ...
      col_types = cols(
        ...
        F_TP = col_character(),
        NO2F = col_double(),
        F_NO2F = col_character(),
        NO3F = col_double(),
        F_NO3F = col_character(),
        TN = col_double(),
        F_TN = col_character()
      )
    )
  ...
  tar_target(
    lksnut14,
    read_csv(lksnut14_file, col_types = cols(F_Record = col_character()))
  ),
  tar_target(
    lksnut15,
    read_csv(lksnut15_file, col_types = cols(F_Record = col_character()))
  ),
  tar_target(
    lksnut16,
    read_csv(lksnut16_file, col_types = cols(F_Record = col_character()))
  ),
  tar_target(
    lksnut17,
    read_csv(lksnut17_file, col_types = cols(F_CHLA_N = col_character()))
  ),
  ...
  tar_target(
    ...
    read_csv(
      lksnut19_file,
      col_types = cols(F_Record = col_character(), F_CHLA_N = col_character())
    )
  ),
  tar_target(
    lksnut20,
    read_csv(
      lksnut20_file,
      col_types = cols(F_Record = col_character(), NH4F = col_double())
    )
  ),
  tar_target(
    lksnut21,
    read_csv(lksnut21_file, col_types = cols(F_Record = col_character()))
  ),
  tar_target(
    lksnut22,
    read_csv(lksnut22_file, col_types = cols(F_Record = col_character()))
  ),
  ...
  tar_target(
    ...
    lksmet %>%
      ...
      mutate(
        DatetimeStamp = mdy_hm(DatetimeStamp, tz = "America/Chicago"),
    ...
    lkswq %>%
      ...
      filter(
        !(is.na(Temp) &
          ...
          is.na(Turb) &
          is.na(ChlFluor))
      ) # filter again to drop more than 10k rows
  ),
  ...
  tar_target(
    ...
    lksnut %>%
      mutate(
        PO4F = qc_lsnerr_nut(PO4F, F_PO4F),
        ...
        NO3F = if_else(
          is.na(NO3F) & str_detect(F_NO3F, "-4"),
          NO23F - NO2F,
          NO3F
        ), # fill in NO3 values that weren't included due to NO2 or NO23 being NDs - if both are ND, we know NO3 is also ND, if NO2 is ND, we know NO3 is basically NO23
  ...
  tar_target(
    ...
    scico %>%
      ...
      mutate(
        ...
        source = if_else(
          site %in% c("lksba", "lksbl", "lksol", "lkspo"),
          "LSNERR",
          "SciCollab"
        ),
        type = "Estuary",
  ...
  tar_target(
    ...
    lksnut_clean %>%
      mutate(
        date = str_split_i(DateTimeStamp, " ", 1),
