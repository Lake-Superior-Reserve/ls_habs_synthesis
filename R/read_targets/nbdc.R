nbdc_targets <- list(
  ...
  #'
  #' Pulls data for a given site from 2010-2025 from NBDC. Also formats dates.
  #'
      ...
      tryCatch(
        {
          nbdc <- read_table(
            nbdc_url,
            skip = 2,
            col_names = nbdc_names,
            show_col_types = FALSE
          ) %>%
            mutate(
              date = str_c(YY, "-", MM, "-", DD, " ", hh, ":", mm),
              date = ymd_hm(date),
        ...
        error = function(e) {
          cat("No stdmet data for", site, "in", year, "\n")
        }
      )
    }
    return(historical)
  ...
  #'
  #' Pulls data for a given site from 2012-2025 from NBDC. Also formats dates. (No data available in 2010-11)
  #'
  #' @param site NBDC site code
  #'
  #' @returns Data frame of data for site
  tar_target(read_nbdc_srad, function(site) {
    ...
    for (year in 2012:2025) {
      nbdc_url <- str_c(
        ...
        "r",
        year,
        ".txt.gz&dir=data/historical/srad/"
      )
      tryCatch(
        {
          nbdc <- read_table(
        ...
        error = function(e) {
          cat("No srad data for", site, "in", year, "\n")
        }
      )
    }
    return(historical)
  ...
  #'
  #' Pulls data for Western Lake Superior buoys from 2010-2025 from NBDC. Then, bind data together
  #'
  ...
  tar_target(
    nbdc,
    full_join(nbdc_stdmet, nbdc_srad, by = join_by(date, site)) %>%
      arrange(date, site)
  ),
