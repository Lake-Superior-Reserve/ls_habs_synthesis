nbdc_targets <- list(
  ...
  tar_target(read_nbdc_stdmet, function(site) {
    ...
    for (year in 2010:2023) {
      ...
      },error=function(e){cat("ERROR: No stdmet data for", site, "in", year, "\n")})
    }
    # get 2024 by month, since historical isn't available yet
    for (m in 1:12) {
      nbdc_url <- str_c("https://www.ndbc.noaa.gov/view_text_file.php?filename=", site, m, "2024", ".txt.gz&dir=data/stdmet/", month(m, label = TRUE), "/")
      tryCatch({
        nbdc <- read_table(nbdc_url, skip = 2, col_names = nbdc_names) %>% 
          mutate(date = str_c(YY, "-", MM, "-", DD, " ", hh, ":", mm),
                 date = ymd_hm(date),
          ...
          relocate(date, site)
        historical <- bind_rows(historical, nbdc)
      },error=function(e){cat("ERROR: No stdmet data for", site, "in", month(m, label = TRUE), "2024", "\n")})
    }
    return(historical)
  }),
    ...
    for (year in 2010:2023) {
      ...
      tryCatch({
        nbdc <- read_table(nbdc_url, skip = 2, col_names = nbdc_names) %>% 
          ...
          relocate(date, site)
        historical <- bind_rows(historical, nbdc)
      },error=function(e){cat("ERROR: No srad data for", site, "in", year, "\n")})
    }
    # get 2024 by month, since historical isn't available yet
    for (m in 1:12) {
      nbdc_url <- str_c("https://www.ndbc.noaa.gov/view_text_file.php?filename=", site, m, "2024", ".txt.gz&dir=data/srad/", month(m, label = TRUE), "/")
      tryCatch({
        nbdc <- read_table(nbdc_url, skip = 2, col_names = nbdc_names) %>% 
          mutate(date = str_c(YY, "-", MM, "-", DD, " ", hh, ":", mm),
                 date = ymd_hm(date),
          ...
          relocate(date, site)
        historical <- bind_rows(historical, nbdc)
      },error=function(e){cat("ERROR: No srad data for", site, "in", month(m, label = TRUE), "2024", "\n")})
    }
    return(historical)
  }),
  ...
  #'
  #' @returns Data frame of data for all sites
  tar_target(nbdc_dulm5_srad, read_nbdc_srad("dulm5")),
  tar_target(nbdc_45027_srad, read_nbdc_srad("45027")),
  tar_target(nbdc_45028_srad, read_nbdc_srad("45028")),
  tar_target(nbdc_45219_srad, read_nbdc_srad("45219")),
  tar_target(nbdc_pngw3_srad, read_nbdc_srad("pngw3")),
  tar_target(nbdc_slvm5_srad, read_nbdc_srad("slvm5")),
  tar_target(nbdc_disw3_srad, read_nbdc_srad("disw3")),
  tar_target(nbdc_sxhw3_srad, read_nbdc_srad("sxhw3")),
  tar_target(nbdc_srad, bind_rows(nbdc_dulm5_srad, nbdc_45027_srad, nbdc_45028_srad, nbdc_45219_srad,
                             nbdc_pngw3_srad, nbdc_slvm5_srad, nbdc_disw3_srad, nbdc_sxhw3_srad)),
  ...
  tar_target(nbdc, full_join(nbdc_stdmet, nbdc_srad) %>% 
               arrange(date, site)),
