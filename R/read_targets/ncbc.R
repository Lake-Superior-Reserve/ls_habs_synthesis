ncbc_targets <- list(
  ...
  tar_target(
    cbnut_file,
    "raw_data/ncbc/Compiled_Bay_Data_2014-2024.xlsx",
    format = "file"
  ),
  tar_target(
    cbtrib_tp_file,
    "raw_data/ncbc/TP_Tribs_14-16.csv",
    format = "file"
  ),
  tar_target(
    cbtrib_ssc_file,
    "raw_data/ncbc/SSC_Tribs_14-16.csv",
    format = "file"
  ),

  tar_target(
    cbq_pco2_file,
    "raw_data/ncbc/discharge/pco2.xlsx",
    format = "file"
  ),
  tar_target(
    cbq_tcbs_file,
    "raw_data/ncbc/discharge/tcbs.csv",
    format = "file"
  ),
  tar_target(
    cbq_sftc_file,
    "raw_data/ncbc/discharge/sftc.csv",
    format = "file"
  ),
  tar_target(
    cbq_nfo2_file,
    "raw_data/ncbc/discharge/nfo2.xlsx",
    format = "file"
  ),
  tar_target(
    cbq_bcbr_file,
    "raw_data/ncbc/discharge/bcbr.csv",
    format = "file"
  ),
  tar_target(
    cbq_nf2i_file,
    "raw_data/ncbc/discharge/nf2i.csv",
    format = "file"
  ),
  tar_target(cbq_bcc_file, "raw_data/ncbc/discharge/bcc.xlsx", format = "file"),
  tar_target(
    cbq_sfcr_file,
    "raw_data/ncbc/discharge/sfcr.csv",
    format = "file"
  ),
  tar_target(
    cbq_ltls_file,
    "raw_data/ncbc/discharge/ltls.csv",
    format = "file"
  ),
  tar_target(
    cbq_nft2_file,
    "raw_data/ncbc/discharge/nft2.csv",
    format = "file"
  ),
  ...
  tar_target(
    cbq_pco2,
    read_xlsx(cbq_pco2_file, skip = 7) %>%
      mutate(site = "PCO2")
  ),
  tar_target(
    cbq_tcbs,
    read_csv(cbq_tcbs_file, skip = 2) %>%
      mutate(site = "TCBS")
  ),
  tar_target(
    cbq_sftc,
  ...
  tar_target(
    cbq_nfo2,
    read_xlsx(cbq_nfo2_file, skip = 14) %>%
      mutate(site = "NFO2")
  ),
  tar_target(
    cbq_bcbr,
  ...
  tar_target(
    cbq_bcc,
    read_xlsx(cbq_bcc_file, skip = 14) %>%
      mutate(site = "BCC")
  ),
  tar_target(
    cbq_sfcr,
  ...
  tar_target(
    ...
    cbtrib_ssc %>%
      mutate(
        ...
        date = mdy_hms(date, tz = "America/Chicago"),
        tss = if_else(
          `Suspended Sediment (1.5 um filter) Concentration (mg/L)` == "ND",
          "0",
          `Suspended Sediment (1.5 um filter) Concentration (mg/L)`
        ),
        tss = as.numeric(tss)
      ) %>%
  ...
  tar_target(
    ...
    cbtrib_tp_daily %>%
      ...
      mutate(
        discharge = rowMeans(across(c(discharge.x, discharge.y)), na.rm = TRUE)
      ) %>%
  ...
  tar_target(
    cbq_clean,
    bind_rows(cbq_s1, cbq_s2, cbq_s3) %>%
