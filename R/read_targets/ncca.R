ncca_targets <- list(
  ...
  tar_target(
    ncca20_sites_file,
    "raw_data/ncca/ncca20-siteinfo-great-lakes-data_revision-1_0.csv",
    format = "file"
  ),
  tar_target(
    ncca15_sites_file,
    "raw_data/ncca/ncca_2015_site_information_great_lakes-data.csv",
    format = "file"
  ),
  tar_target(
    ncca10_sites_file,
    "raw_data/ncca/assessed_ncca2010_siteinfo.revised.06212016.csv",
    format = "file"
  ),
  tar_target(
    ncca10na_sites_file,
    "raw_data/ncca/not_assessed_ncca2010_siteinfo.revised.06212016.csv",
    format = "file"
  ),

  tar_target(
    ncca20_secchi_file,
    "raw_data/ncca/ncca20_secchi_data.csv",
    format = "file"
  ),
  tar_target(
    ncca15_secchi_file,
    "raw_data/ncca/ncca_2015_secchi_great_lakes-data.csv",
    format = "file"
  ),

  tar_target(
    ncca20_hydro_file,
    "raw_data/ncca/ncca20_hydroprofile_data.csv",
    format = "file"
  ),
  tar_target(
    ncca15_hydro_file,
    "raw_data/ncca/ncca_2015_hydrographic_profile_great_lakes-data.csv",
    format = "file"
  ),
  tar_target(
    ncca10_hydro_file,
    "raw_data/ncca/assessed_ncca2010_hydrolab.csv",
    format = "file"
  ),
  tar_target(
    ncca10na_hydro_file,
    "raw_data/ncca/not_assessed_ncca2010_hydrolab.csv",
    format = "file"
  ),

  tar_target(
    ncca20_chem_file,
    "raw_data/ncca/ncca20_waterchem_data.csv",
    format = "file"
  ),
  tar_target(
    ncca15_chem_file,
    "raw_data/ncca/ncca_2015_water_chemistry_great_lakes-data.csv",
    format = "file"
  ),
  tar_target(
    ncca10_chem_file,
    "raw_data/ncca/assessed_ncca2010_waterchem.csv",
    format = "file"
  ),
  tar_target(
    ncca10na_chem_file,
    "raw_data/ncca/not_assessed_ncca2010_waterchem.csv",
    format = "file"
  ),
  ...
  tar_target(
    ncca20_sites,
    read_csv(ncca20_sites_file) %>%
      filter(
        GREAT_LAKE == "Lake Superior" &
          LAT_DD83 < 47.5 &
          LON_DD83 < -90 &
          VISIT_NO == 1
      )
  ...
  tar_target(
    ncca15_sites,
    read_csv(ncca15_sites_file) %>%
      filter(
        FEAT_NM == "Lake_Superior" &
          LAT_DD83 < 47.5 &
          LON_DD83 < -90 &
          VISIT_NO == 1
      )
  ...
  tar_target(
    ncca20_secchi,
    read_csv(ncca20_secchi_file) %>%
      filter(SITE_ID %in% ncca20_sites$SITE_ID)
  ),
  tar_target(
    ncca15_secchi,
    read_csv(ncca15_secchi_file) %>%
      filter(SITE_ID %in% ncca15_sites$SITE_ID)
  ),

  tar_target(
    ncca20_hydro,
    read_csv(ncca20_hydro_file) %>%
      filter(SITE_ID %in% ncca20_sites$SITE_ID)
  ),
  tar_target(
    ncca15_hydro,
    read_csv(ncca15_hydro_file) %>%
      filter(SITE_ID %in% ncca15_sites$SITE_ID)
  ),
  tar_target(
    ncca10_hydro,
    read_csv(ncca10_hydro_file) %>%
      filter(SITE_ID %in% ncca10_sites$SITE_ID)
  ),
  tar_target(
    ncca10na_hydro,
    read_csv(ncca10na_hydro_file) %>%
      filter(SITE_ID %in% ncca10na_sites$SITE_ID)
  ),

  tar_target(
    ncca20_chem,
    read_csv(ncca20_chem_file) %>%
      filter(SITE_ID %in% ncca20_sites$SITE_ID)
  ),
  tar_target(
    ncca15_chem,
    read_csv(ncca15_chem_file) %>%
      filter(SITE_ID %in% ncca15_sites$SITE_ID)
  ),
  tar_target(
    ncca10_chem,
    read_csv(ncca10_chem_file) %>%
      filter(SITE_ID %in% ncca10_sites$SITE_ID)
  ),
  tar_target(
    ncca10na_chem,
    read_csv(ncca10na_chem_file) %>%
      filter(SITE_ID %in% ncca10na_sites$SITE_ID)
  ),
  ...
  tar_target(
    ...
    ncca10_sites %>%
      ...
      mutate(
        ncca.x = if_else(is.na(ncca.x) & !is.na(ncca.y), ncca.y, ncca.x)
      ) %>%
      select(
        ncca = ncca.x,
        site = site.x,
  ...
  tar_target(
    ...
    ncca20_chem %>%
      mutate(
        RESULT = if_else(is.na(RESULT) & NARS_FLAG == "ND", 0.5 * MDL, RESULT),
        DATE_COL = mdy(DATE_COL)
      ) %>%
      select(DATE_COL, SITE_ID, ANALYTE, RESULT) %>%
      pivot_wider(names_from = ANALYTE, values_from = RESULT) %>%
      mutate(
        NITRATE_N = if_else(is.na(NITRATE_N), NITRATE_NITRITE_N, NITRATE_N)
      ) %>% # count nitrate/nitrite as nitrate
      select(
        date = DATE_COL,
        site = SITE_ID,
        chl = CHLA,
  ...
  tar_target(
    ...
    ncca10_chem %>%
      mutate(
        RESULT = if_else(!is.na(QACODE) & QACODE == "N", 0.5 * MDL, RESULT),
        DATE_COL = mdy(DATE_COL)
      ) %>%
  ...
  tar_target(
    ...
    ncca_chem %>%
      ...
      mutate(
        site = if_else(!is.na(ncca), ncca, site),
        source = "NCCA",
        type = if_else(
          site %in%
            c(
