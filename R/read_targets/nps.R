nps_targets <- list(
  ...
  tar_target(
    nps16_sonde_file,
    "raw_data/nps/Apostle Islands Sonde Data.csv",
    format = "file"
  ),
  tar_target(nps_sonde_file, "raw_data/nps/sonde.csv", format = "file"),
  tar_target(
    nps_chem_file,
    "raw_data/nps/Apostle Islands Water Chemistry.csv",
    format = "file"
  ),
  ...
  tar_target(
    nps16_sonde_deploy_clean,
    nps16_sonde %>% # continuous data at consistent depth
      mutate(
        site = Station,
  ...
  tar_target(
    nps16_sonde_profile_clean,
    nps16_sonde %>% # discrete data at multiple depths
      mutate(
        site = paste("SIW", Transect_Station, sep = " "),
        date = parse_date_time(
          `Date/Time`,
          c("%m/%d/%Y"),
          exact = TRUE,
  ...
  tar_target(
    ...
    nps_wetlab %>%
      mutate(
        date = mdy_hm(DateTime, tz = "America/Chicago"),
  ...
  tar_target(
    ...
    nps_sonde %>%
      ...
      mutate(
        date = mdy_hm(date, tz = "America/Chicago"),
  ...
  tar_target(
    ...
    nps_sonde_clean %>%
      ...
      summarise(across(
        c(depth, chl, temp, do, do_sat, ph, cond, turb, tds),
        ~ mean(.x, na.rm = TRUE)
      )) %>%
      ungroup()
  ),
  ...
  ), # when depth is not specified, assume surface
  ...
  tar_target(
    ...
    nps_chem_surf %>%
      ...
      rename(
        latitude = latitude.x, # using coordinates from chem file, since they're consistent with other sonde data
