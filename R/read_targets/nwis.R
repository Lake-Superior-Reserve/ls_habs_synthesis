nwis_targets <- list(
        ...
        !(
          site_no %in%
            c(
              ...
              "470535092570801",
              "471149092360901",
              "04021520", # these 5 are upstream in St Louis watershed
              "04027200",
              "040265935",
              "040265981",
              "04026740",
              ...
              "04026511",
              "04026561",
              "04026900", # these are too far upstream in bad river area
              "04029990",
              "04001000"
            ) # too far east; isle royale
        )
      )
  ),
  ...
  tar_target(
    ...
    readNWISdata(
      service = "dv",
      siteNumbers = unique(filter(nwis_metadata, data_type_cd == "dv")$site_no),
  ...
  tar_target(
    ...
    readNWISdata(
      service = "iv",
      siteNumbers = unique(filter(nwis_metadata, data_type_cd == "uv")$site_no),
  ...
  tar_target(
    ...
    nwis_pull_dv %>%
      ...
      filter(
        !if_all(
          c(discharge, temp, cond, do, ph, turb, tss, tkn, no3, tp),
          is.na
        )
      )
  ),
  ...
  tar_target(
    ...
    nwis_uv_clean %>%
      ...
      group_by(date, site) %>%
      summarise(across(
        c(discharge, temp, cond, do, ph, turb),
        ~ mean(.x, na.rm = TRUE)
      )) %>%
      ungroup() %>%
  ...
  tar_target(
    nwis,
    bind_rows(nwis_dv_clean, nwis_uv_daily) %>%
