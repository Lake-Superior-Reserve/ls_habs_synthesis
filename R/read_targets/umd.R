umd_targets <- list(
  ...
  tar_target(
    umd_1721_file,
    "raw_data/umd/SouthShoreArchive.csv",
    format = "file"
  ),
  tar_target(
    umd_2123_file,
    "raw_data/umd/SouthShoreArchive2021to2023.csv",
    format = "file"
  ),
  ...
  tar_target(
    ...
    umd_1721 %>%
      ...
      summarise(
        ...
        type = first(Type),
        doc = mean(DOC, na.rm = T),
        poc = mean(POC, na.rm = T),
        poc_filt = mean(POC2, na.rm = T),
        no3 = mean(NO3, na.rm = T),
        nh3 = mean(NH3, na.rm = T),
        tdn = mean(TDN, na.rm = T),
        pon = mean(PON, na.rm = T),
        pon_filt = mean(PON2, na.rm = T),
        po4 = mean(SRP, na.rm = T),
        tdp = mean(TDP, na.rm = T),
        pp = mean(PP, na.rm = T),
        tp = mean(TP, na.rm = T),
        si = mean(SRSi, na.rm = T),
        chl = mean(Chl, na.rm = T),
        chl_filt = mean(Chl2, na.rm = T),
        phyco = mean(Phyco, na.rm = T),
      ...
      mutate(
        ...
        doc = doc * .001 * 12.011,
        poc = poc * .001 * 12.011,
        poc_filt = poc_filt * .001 * 12.011, #converting units from to umol/L to mg/L by converting to mmol/L then multiplying by molar mass
        no3 = no3 * .001 * 14.007,
        nh3 = nh3 * .001 * 14.007,
        tdn = tdn * .001 * 14.007,
        pon = pon * .001 * 14.007,
        pon_filt = pon_filt * .001 * 14.007,
        po4 = po4 * .001 * 30.974,
        tdp = tdp * .001 * 30.974,
        pp = pp * .001 * 30.974,
        tp = tp * .001 * 30.974,
  ...
  tar_target(
    ...
    umd_2123 %>%
      ...
      mutate(
        si = si * .001 * 28.086,
        doc = doc * .001 * 12.011,
        poc = poc * .001 * 12.011, #converting units from to umol/L to mg/L by converting to mmol/L then multiplying by molar mass
        no3 = no3 * .001 * 14.007,
        nh3 = nh3 * .001 * 14.007,
        tdn = tdn * .001 * 14.007,
        pon = pon * .001 * 14.007,
  ...
  tar_target(clean_umd20_troll, function(df) {
    df %>%
      select(
        ...
        latitude = `Latitude (°)`,
        longitude = `Longitude (°)`,
        depth = `Depth (ft) (525639)`,
        chl_field = `Chlorophyll-a Concentration (µg/L) (652536)`,
        temp = `Water Temperature (°C) (519767)`,
        cond = `Specific Conductivity (µS/cm) (675375)`,
        turb = `Turbidity (NTU) (695981)`,
        tds = `Total Dissolved Solids (ppt) (675375)`,
        tss = `Total Suspended Solids (mg/L) (695981)`
      ) %>%
      mutate(
        tds = tds * 1000, #convert to uS/cm
  ...
  tar_target(
    ...
    umd23_troll %>%
      mutate(
        date = force_tz(`Date Time`, tzone = "America/Chicago"),
      ...
      select(
        ...
        longitude = `Longitude (°)`,
        depth,
        chl_field = `Chlorophyll-a Fluorescence (RFU) (804408)`,
        temp = `Temperature (°C) (804550)`, # based on the troll 2020 file, chl RFU is equivalent to ug/L for this sensor
        cond = `Specific Conductivity (µS/cm) (803397)`,
        turb = `Turbidity (NTU) (803671)`,
        tds
      ) %>%
      mutate(
        cond = if_else(cond < 50, NA, cond), # drop very low/high data
  ...
  tar_target(
    umd20_troll_profile_surf,
    umd20_troll_profile %>%
      filter(depth <= 2) %>% # only want surface measurements
      mutate(date = date(date)) %>%
      group_by(date, site) %>%
      summarise(across(
        c(latitude, longitude, depth, chl_field, temp, cond, turb, tds, tss),
        ~ mean(.x, na.rm = TRUE)
      )) %>%
      ungroup()
  ),
  tar_target(
    umd20_troll_transect_surf,
    umd20_troll_transect %>%
      filter(depth <= 2) %>% # only want surface measurements
      mutate(date = date(date)) %>%
      group_by(date, site) %>%
  ...
  tar_target(
    ...
    umd23_troll_clean %>%
      ...
      group_by(date, site) %>%
      summarise(across(
        c(latitude, longitude, depth, chl_field, temp, cond, turb, tds),
        ~ mean(.x, na.rm = TRUE)
      )) %>%
      ungroup() %>%
      mutate(
        site = sapply(site, function(site) {
          if (site == "BBay") {
            return("Bark Bay")
          } else if (site == "MB5a") {
            return("Mawikwe Bay")
          } else if (site == "MB5c") {
            return("Mawikwe Bay c")
          } else if (site == "SBay") {
            return("Siskiwit Bay")
          } else if (site == "SBay2") {
            return("Siskiwit Bay 2")
          } else if (site == "SBay3") {
            return("Siskiwit Bay 3")
          } else if (site == "SBay1") {
            return("Siskiwit Bay")
          } else if (site == "SCav") {
            return("Sea Caves")
  ...
  tar_target(
    umd,
    bind_rows(umd_1721_clean, umd_2123_clean) %>%
      mutate(across(
        -c(source, type, site, depth, latitude, longitude),
        replace_nan
      )) %>%
      left_join(
        select(umd_troll_surf, -c(depth, tss, latitude, longitude)),
        by = join_by(date, site)
      ) %>%
      mutate(
        type = if_else(
          str_detect(site, "Barkers") | site == "Nemadji River ISCO",
          "Estuary",
          type
        ),
