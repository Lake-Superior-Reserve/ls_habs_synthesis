...
dnr <- tar_read(dnr)
ls_crop_dnr <- st_crop(tar_read(ls_shp), st_bbox(c(xmin = -92, ymin = 46.6, xmax = -91, ymax = 46.9)))
...
dnr_site_clus_kmean <- kmeans(dnr_site_clus_prep, centers = 2, nstart = 25)
dnr_rf_prep <- dnr %>% 
               st_drop_geometry() %>% 
...
fviz_cluster(dnr_site_clus_kmean, data = dnr_site_clus_prep)
```
